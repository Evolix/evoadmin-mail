...
wget -q -t 3 http://zidane.evolix.net/spam/client.access -O $tmp_file
cp $tmp_file /etc/postfix/client.access
rm $tmp_file

wget -q -t 3 http://zidane.evolix.net/spam/sender.access -O $tmp_file
cp $tmp_file /etc/postfix/sender.access
rm $tmp_file

wget -q -t 3 http://zidane.evolix.net/spam/recipient.access -O $tmp_file
cp $tmp_file /etc/postfix/recipient.access
rm $tmp_file

wget -q -t 3 http://zidane.evolix.net/spam/header_kill -O $tmp_file
cp $tmp_file /etc/postfix/header_kill
rm $tmp_file

wget -q -t 3 http://zidane.evolix.net/spam/sa-blacklist.access  -O sa-blacklist.access
wget -q -t 3 http://zidane.evolix.net/spam/sa-blacklist.access.md5  -O $tmp_file
...
wget -q -t 3 http://zidane.evolix.net/spam/spamd.cidr  -O spamd.cidr
wget -q -t 3 http://zidane.evolix.net/spam/spamd.cidr.md5  -O $tmp_file
...
#    getfile http://zidane.evolix.net/spam/$file $file
#done
#chown -R clamav:clamav /var/lib/clamav
#/usr/sbin/invoke-rc.d clamav-daemon reload-database > /dev/null

# SpamAssassin
wget -q -t 3 http://zidane.evolix.net/spam/evolix_rules.cf -O evolix_rules.cf
